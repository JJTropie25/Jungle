...
export default function HostEditListing() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setDescription(
        service.description ??
          `${service.title} near ${service.location} is designed for fast, reliable access with clear check-in flow, secure space, and flexible timing.`
      );
  ...
  const onSave = async () => {
    ...
    setSaving(false);
    if (err) {
      await dialog.alert(t("host.listings.editTitle"), err);
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: insets.top + 16 }]}>
        ...
        <Text style={styles.title}>{t("host.listings.editTitle")}</Text>
        {loading ? <Text style={styles.loading}>{t("host.loading")}</Text> : null}
